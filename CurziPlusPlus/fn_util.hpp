...
#define LIFT_FILTER_X(operand_name_here, ...) std::views::filter([](const auto& operand_name_here) { return __VA_ARGS__ ; })
#define LIFT_TRANSFORM_X(operand_name_here, ...) std::views::transform([](const auto& operand_name_here) { return __VA_ARGS__ ; })
